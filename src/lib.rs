...
impl CompactSize {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let prefix = bytes[0];
        match prefix {
            0..=0xFC => Ok((Self::new(prefix as u64), 1)),
            0xFD => {
                if bytes.len() < 3 {
                    return Err(BitcoinError::InsufficientBytes);
                }
...
impl Script {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        Ok((Self::new(script_bytes), consumed + len))
    }
}
...
impl BitcoinTransaction {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let inputs_size = CompactSize::new(self.inputs.len() as u64);
        result.extend(inputs_size.to_bytes());
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (inputs_size, consumed) = CompactSize::from_bytes(&bytes[offset..])?;
        offset += consumed;
        let count = inputs_size.value as usize;
